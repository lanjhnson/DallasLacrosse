...
class GFPayPalPro {
    ...
    private static $version = "1.0.beta4";
    ...
    public static function plugin_row(){
        if(!self::is_gravityforms_supported()){
            ...
            if(!$version_info["is_valid_key"]){
                $new_version = version_compare(self::$version, $version_info["version"], '<') ? __('There is a new version of Gravity Forms PayPal Payments Pro Add-On available.', 'gravityformspaypalpro') .' <a class="thickbox" title="Gravity Forms PayPal Payments Pro Add-On" href="plugin-install.php?tab=plugin-information&plugin=' . self::$slug . '&TB_iframe=true&width=640&height=808">'. sprintf(__('View version %s Details', 'gravityformspaypalpro'), $version_info["version"]) . '</a>. ' : '';
    ...
    private static function list_page(){
        if(!self::is_gravityforms_supported()){
            die(__(sprintf("PayPal Payments Pro Add-On requires Gravity Forms %s. Upgrade automatically on the %sPlugin page%s.", self::$min_gravityforms_version, "<a href='plugins.php'>", "</a>"), "gravityformspaypalpro"));
        }
                        ...
                        if(!$is_settings_configured){
                            ...
                            <tr>
                                <td colspan="4" style="padding:20px;">
                                    <?php echo sprintf(__("To get started, please configure your %sPayPal Payments Pro Settings%s.", "gravityformspaypalpro"), '<a href="admin.php?page=gf_settings&addon=PayPal Pro">', "</a>"); ?>
                                </td>
                            </tr>
                            <?php
                        }
                            ...
                            <tr>
                                <td colspan="4" style="padding:20px;">
                                    <?php echo sprintf(__("You don't have any PayPal Payments Pro feeds configured. Let's go %screate one%s!", "gravityformspaypalpro"), '<a href="admin.php?page=gf_paypalpro&view=edit&id=0">', "</a>"); ?>
                                </td>
                            </tr>
                            <?php
                        }
    ...
    public static function settings_page(){
        ...
        if(rgpost("uninstall")){
            ...
            ?>
            <div class="updated fade" style="padding:20px;"><?php _e(sprintf("Gravity Forms PayPal Payments Pro Add-On have been successfully uninstalled. It can be re-activated from the %splugins page%s.", "<a href='plugins.php'>","</a>"), "gravityformspaypalpro")?></div>
            <?php
            return;
        }
        ...
        $message = "";
        if($is_valid)
            $message = __("Valid PayPal Payments Pro credentials.", "gravityformspaypalpro");
        else if(!empty($settings["username"]))
            $message = __("Invalid PayPal Payments Pro credentials.", "gravityformspaypalpro");
        ...
        <form action="" method="post">
            ...
            <table class="form-table">
                <tr>
                    <td colspan="2">
                        <h3><?php _e("PayPal Payments Pro Settings", "gravityformspaypalpro") ?></h3>
                ...
                <tr>
                    <td colspan="2">
                        ...
                        <p style="text-align: left;">
                            <?php _e("To create recurring payments, you must have Instant Payment Notification (IPN) setup in your PayPal Payments Pro account. Follow the steps below and confirm IPN is enabled.", "gravityformspaypalpro") ?>
                        </p>
                        <ul>
                ...
                <tr>
                    <td colspan="2">
                        ...
                        <label for="gf_ipn_configured" class="inline"><?php _e("IPN is setup in my PayPal Payments Pro account.", "gravityformspaypalpro") ?></label>
                    </td>
                </tr>
                <tr>
        ...
        <form action="" method="post">
            ...
            <?php if(GFCommon::current_user_can_any("gravityforms_paypalpro_uninstall")){ ?>
                ...
                <h3><?php _e("Uninstall PayPal Payments Pro Add-On", "gravityformspaypalpro") ?></h3>
                <div class="delete-alert"><?php _e("Warning! This operation deletes ALL PayPal Payments Pro Feeds.", "gravityformspaypalpro") ?>
                    <?php
                    $uninstall_button = '<input type="submit" name="uninstall" value="' . __("Uninstall PayPal Payments Pro Add-On", "gravityformspaypalpro") . '" class="button" onclick="return confirm(\'' . __("Warning! ALL PayPal Payments Pro Feeds will be deleted. This cannot be undone. \'OK\' to delete, \'Cancel\' to stop", "gravityformspaypalpro") . '\');"/>';
    ...
    private static function post_to_paypal($methodName, $nvp = array()) {
    ...
    private static function stats_page(){
        ...
        <div class="wrap">
            ...
            <form method="post" action="">
                ...
                $transaction_totals = GFPayPalProData::get_transaction_totals($config["form_id"]);
    ...
    public static function cancel_paypalpro_subscription() {
        ...
        $response = self::post_to_paypal("ManageRecurringPaymentsProfileStatus", $args);
        if(!empty($response) && $response["ACK"] == "Success")
        {
    ...
    private static function cancel_subscription($lead){
        ...
        $feed_id = gform_get_meta($lead["id"], "paypalpro_feed_id");
    ...
    private static function edit_page(){
        ...
        <div class="wrap">
            <img alt="<?php _e("PayPal Pro", "gravityformspaypalpro") ?>" style="margin: 15px 7px 0pt 0pt; float: left;" src="<?php echo self::get_base_url() ?>/images/paypal_wordpress_icon_32.png"/>
            <h2><?php _e("PayPal Payments Pro Transaction Settings", "gravityformspaypalpro") ?></h2>
        ...
        if(rgpost("gf_paypalpro_submit")){
            ...
            $config["meta"]["trial_period_enabled"] = rgpost('gf_paypalpro_trial_period');
            $config["meta"]["trial_type"] = rgpost('gf_paypalpro_trial_type');
            $config["meta"]["trial_amount_field"] = rgpost('gf_paypalpro_trial_amount');
            $config["meta"]["trial_period_number"] = rgpost('gf_paypalpro_trial_period_number');
            $config["meta"]["trial_period_type"] = rgpost('gf_paypalpro_trial_period_type');
            $config["meta"]["trial_recurring_times"] = rgpost("gf_paypalpro_trial_recurring_times");
            ...
            <div id="paypalpro_field_group" valign="top" <?php echo empty($config["meta"]["type"]) || empty($config["form_id"]) ? "style='display:none;'" : "" ?>>
                ...
                <div id="paypalpro_field_container_subscription" class="paypalpro_field_container" valign="top" <?php echo rgars($config, "meta/type") != "subscription" ? "style='display:none;'" : ""?>>
                    ...
                    <div class="margin_vertical_10">
                        ...
                        <input type="checkbox" name="gf_paypalpro_trial_period" id="gf_paypalpro_trial_period" value="1" onclick="if(jQuery(this).is(':checked')) jQuery('#paypalpro_trial_period_container').show('slow'); else jQuery('#paypalpro_trial_period_container').hide('slow');" <?php echo rgars($config, "meta/trial_period_enabled") ? "checked='checked'" : ""?> />
                        <label class="inline" for="gf_paypalpro_trial_period"><?php _e("Enable", "gravityformspaypalpro"); ?></label>
                    </div>

                    <div id="paypalpro_trial_period_container" <?php echo rgars($config, "meta/trial_period_enabled") ? "" : "style='display:none;'" ?>>
                    ...
                    <div class="margin_vertical_10">
                        ...
                        <input type="checkbox" onchange="if(this.checked) {jQuery('#gf_paypalpro_setup_fee_amount').val('Select a field');}" name="gf_paypalpro_setup_fee" id="gf_paypalpro_setup_fee" value="1" onclick="if(jQuery(this).is(':checked')) jQuery('#paypalpro_setup_fee_container').show('slow'); else jQuery('#paypalpro_setup_fee_container').hide('slow');" <?php echo rgars($config, "meta/setup_fee_enabled") ? "checked='checked'" : ""?> />
    ...
    public static function get_config($form){
        if(!class_exists("GFPayPalProData"))
            require_once(self::get_base_path() . "/data.php");
    ...
    public static function paypalpro_validation($validation_result){
        ...
        $products = GFCommon::get_product_fields($validation_result["form"], $lead);
        ...
        foreach($products["products"] as $product_id => $product)
        {
            ...
            $amount += ($product_amount * $product["quantity"]);

            //adding line items
            if($config["meta"]["type"] == "product"){
                $billing['L_NAME'.$item] = $product["name"];
                $billing['L_AMT'.$item]  = $product_amount;
                $billing['L_NUMBER'.$item] = $item+1;
                $billing['L_QTY'.$item] = $product["quantity"];
            }
            else
            {
                //adding recurring description
                $billing['DESC'] .=  $item > 1 ? ", " . $product["name"] : $product["name"];
            }
        ...
        if(!empty($products["shipping"]["name"]) && ($config["meta"]["type"] == "product" || $recurring_amount_field == "all")){
            if($config["meta"]["type"] == "product"){
                $billing['L_NAME'.$item] = $products["shipping"]["name"];
                $billing['L_AMT'.$item]  = $products["shipping"]["price"];
                $billing['L_NUMBER'.$item] = $item+1;
                $billing['L_QTY'.$item] = 1;
            }
        ...
        if($config["meta"]["type"] == "product"){
            ...
            $response = self::post_to_paypal("DoDirectPayment",$billing);

            if(!empty($response) && !empty($response["TRANSACTIONID"])){
                self::$transaction_response = array("transaction_id" => $response["TRANSACTIONID"], "amount" => $response["AMT"], "transaction_type" => 1);
                return $validation_result;
            }
            else
            {
                // Payment was not succesfull, need to display error message
        ...
        else
        {
            ...
            if($config["meta"]["setup_fee_enabled"])
            {
                ...
                if(!empty($setup_fee_product)){
                    $billing['INITAMT'] = self::get_product_price($setup_fee_product);
                }
            }
            ...
            if($config["meta"]["trial_period_enabled"] )
            {
                if($config["meta"]["trial_type"] == "paid")
                {
                    ...
                    $billing["TRIALAMT"] = empty($trial_product) ? 0 : self::get_product_price($trial_product);
                }
            ...
            $response = self::post_to_paypal("CreateRecurringPaymentsProfile",$billing);
             if(!empty($response) && !empty($response["PROFILEID"])){
                self::$transaction_response = array("transaction_id" => $response["PROFILEID"], "amount" => 0, "transaction_type" => 2 );
                return $validation_result;
            }
            else
            {
    ...
    private static function set_validation_result($validation_result,$post,$response,$responsetype){
        ...
        {
            $code = $response["L_ERRORCODE0"];
                $message = __("<!-- Subscription Error: " . $code . " -->There was an error processing your request. Your credit card was not charged. Please try again.", "gravityforms");
        }
    ...
    public static function paypalpro_after_submission($entry,$form){
        ...
        //abort if a payment gateway transaction wasn't created
        if(empty(self::$transaction_response))
            return;

        //saving payment gateway in meta
        gform_update_meta($entry["id"], "payment_gateway", "paypalpro");

        //saving feed id in meta
        $config = self::get_config($form);
        gform_update_meta($entry["id"], "paypalpro_feed_id", $config["id"]);
        ...
        $entry["payment_status"] = $transaction_type == "1" ? "Approved" : "Active";
        ...
        $subscriber_id = $transaction_type == "2" ? $transaction_id : "";
        GFPayPalProData::insert_transaction($entry["id"], $config["id"], "payment", $subscriber_id, $transaction_id, "", $amount);
    }
    ...
    public static function set_payment_status($config, $entry, $status, $transaction_type, $transaction_id, $subscriber_id, $amount, $profile_status, $period_type, $initial_payment_status, $initial_payment_amount, $initial_payment_transaction_id, $parent_transaction_id, $reason_code){
        ...
        switch(strtolower($transaction_type)){
            case "recurring_payment_profile_created":
                if($initial_payment_status == "Completed" && $initial_payment_amount <> 0)
                    RGFormsModel::add_note($entry["id"], $user_id, $user_name, sprintf(__("Setup Fee payment has been made. Amount: %s. Transaction Id: %s", "gravityformspaypalpro"), GFCommon::to_money($initial_payment_amount, $entry["currency"]), $initial_payment_transaction_id));
                    GFPayPalProData::insert_transaction($entry["id"], $config["id"], "payment", $subscriber_id, $initial_payment_transaction_id, $parent_transaction_id, $initial_payment_amount);
            break;
                ...
                if($amount <> 0){
                    if($profile_status == "Active") {
                        ...
                        if($period_type == "Trial")
                            RGFormsModel::add_note($entry["id"], $user_id, $user_name, sprintf(__("Trial payment has been made. Amount: %s. Transaction Id: %s", "gravityforms"), GFCommon::to_money($amount, $entry["USD"]), $transaction_id));
                        else
                            RGFormsModel::add_note($entry["id"], $user_id, $user_name, sprintf(__("Subscription payment has been made. Amount: %s. Transaction Id: %s", "gravityforms"), GFCommon::to_money($amount, $entry["USD"]), $transaction_id));
    ...
    public static function uninstall(){
        ...
        if(!GFPayPalPro::has_access("gravityforms_paypalpro_uninstall"))
            die(__("You don't have adequate permission to uninstall the PayPal Payments Pro Add-On.", "gravityformspaypalpro"));
